...
#!/usr/bin/env python3
# compare_segment_with_raw.py (v3)
#
# PURPOSE
...
#
# RAW SERVER DATA LAYOUT (under --raw_root):
# /storage/lukovic/Data/FORWARDS/treenet/server_data/
# ├─ thermometer_l1/thermometer_l1_series_id_<ID>.ftr   (10‑min) columns = ['ts', 'value']
# ├─ hygrometer_l1/hygrometer_l1_series_id_<ID>.ftr     (10‑min) columns = ['ts', 'value']
# ├─ dendrometer_l2/dendrometer_l2_series_id_<ID>.ftr   (10‑min) columns = ['ts', 'value']
# ├─ dendrometer_lm/dendrometer_lm_series_id_<ID>.ftr   (10‑min) columns = ['ts', 'value', 'rh', 'temp']
# └─ meteo_data/site_<SITE>.csv                         (daily)   columns = ['ts','tas','tasmax','tasmin','rh','vpd','gh','pr']
#
import os
import argparse
import pickle
...
# ---------------- raw loaders ----------------
def _read_ftr_columns(raw_path: str, wanted_cols: List[str]) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"{raw_path} missing expected columns: {missing}; has {list(df.columns)}")
    out = df[wanted_cols].astype('float64')
    out.index = ts
    return out.sort_index()

def read_thermometer_l1(raw_root: str, thermo_id: int) -> pd.DataFrame:
    path = os.path.join(raw_root, 'thermometer_l1', f'thermometer_l1_series_id_{thermo_id}.ftr')
    return _read_ftr_columns(path, ['value'])

def read_hygrometer_l1(raw_root: str, hygro_id: int) -> pd.DataFrame:
    path = os.path.join(raw_root, 'hygrometer_l1', f'hygrometer_l1_series_id_{hygro_id}.ftr')
    return _read_ftr_columns(path, ['value'])

def read_dendrometer_l2(raw_root: str, dendro_id: int) -> pd.DataFrame:
    path = os.path.join(raw_root, 'dendrometer_l2', f'dendrometer_l2_series_id_{dendro_id}.ftr')
    return _read_ftr_columns(path, ['value'])

def read_dendrometer_lm(raw_root: str, lm_dendro_id: int) -> pd.DataFrame:
    path = os.path.join(raw_root, 'dendrometer_lm', f'dendrometer_lm_series_id_{lm_dendro_id}.ftr')
    return _read_ftr_columns(path, ['value','rh','temp'])

def read_site_csv(raw_root: str, site_id: int, year: int) -> pd.DataFrame:
    path = os.path.join(raw_root, 'meteo_data', f'site_{site_id}.csv')
    if not os.path.isfile(path):
        raise RuntimeError(f"Site CSV not found: {path}")
    df = pd.read_csv(path)
    ts_col = 'ts' if 'ts' in df.columns else ('ts_local' if 'ts_local' in df.columns else None)
    if ts_col is None:
        raise RuntimeError(f"Site CSV missing 'ts'/'ts_local': {path}")
...
def doy_from_index(idx) -> np.ndarray:
    """
    Return *fractional* Day-Of-Year in LOCAL_TZ so that sub-daily sampling
    (10-min, hourly) appears with native resolution on the x-axis.
    """
    ...
    # Fallback: Series/array-like of timestamps
    ts = pd.to_datetime(idx, errors='coerce')
    if getattr(ts.dt, 'tz', None) is None:
        ts = ts.dt.tz_localize(LOCAL_TZ, nonexistent='shift_forward', ambiguous='NaT')
    else:
        ts = ts.dt.tz_convert(LOCAL_TZ)
    seconds = (ts.dt.hour * 3600 + ts.dt.minute * 60 + ts.dt.second) + (ts.dt.microsecond / 1e6)
    frac = seconds / 86400.0
    return ts.dt.dayofyear.to_numpy(dtype=float) + frac
...
def compare_one_segment(
    ...
    seg_idx: int,
    ids_row,
    in_min: Dict, in_diff: Dict,
    out_min: Dict, out_diff: Dict,
    ws_utc: pd.Timestamp, we_utc: pd.Timestamp,
    Xseg_norm: pd.DataFrame, Yseg_norm: pd.DataFrame,
    raw_temp: pd.DataFrame, raw_hygro: pd.DataFrame,
    raw_l2: pd.DataFrame, raw_lm: pd.DataFrame, raw_site: pd.DataFrame,
    fig_out_dir: str,
):
    # Denorm helpers
    ...
    df_site_w = raw_site[
        (raw_site['ts_local'] >= ws_utc.tz_convert(LOCAL_TZ)) &
        (raw_site['ts_local'] <  we_utc.tz_convert(LOCAL_TZ))
    ]

    # Resampling for targets (hourly means in local time -> back to UTC)
    def to_hourly_mean(df: pd.DataFrame) -> pd.DataFrame:
        if df.empty:
            return df
        loc = df.copy()
        loc.index = loc.index.tz_convert(LOCAL_TZ)
        hourly = loc.resample('h').mean()
    ...
    def seg_daily_mean(df: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
        ...
        loc = df.copy()
        loc.index = loc.index.tz_convert(LOCAL_TZ)
        return loc[use].resample('D').mean()

    Xseg_daily = seg_daily_mean(Xseg_orig, GLOBAL_CHANNELS)

    # Plotting
    out_dir = os.path.join(fig_out_dir, f"year_{year}", f"site_{site_id}", f"combo_{combo_id}", f"seg_{seg_idx}")
    ensure_dir(out_dir)

    def plot_two_lines(x1, y1, label1, x2, y2, label2, ch):
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.plot(doy_from_index(x1), y1, color='tab:blue', lw=1.2, alpha=0.95, label=label1)
        ax.plot(doy_from_index(x2), y2, color='red',   lw=2.0, alpha=0.90, linestyle='--', label=label2)
        ...
        all_doy = np.concatenate([doy_from_index(x1), doy_from_index(x2)]) if len(x1) and len(x2) else np.array([])
        if all_doy.size > 0:
        ...
        title = (
            ...
            f"Window: {ws_utc.tz_convert(LOCAL_TZ).strftime('%Y-%m-%d %H:%M')} → "
            f"{we_utc.tz_convert(LOCAL_TZ).strftime('%Y-%m-%d %H:%M')} (local)"
        )
    ...
    # Inputs
    if 'temp_treenet' in xin_cols and not df_temp_w.empty and 'temp_treenet' in Xseg_orig.columns:
        plot_two_lines(df_temp_w.index, df_temp_w['value'].to_numpy(), 'raw thermometer L1',
                       Xseg_orig.index, Xseg_orig['temp_treenet'].to_numpy(), 'segment denorm', 'temp_treenet')
    if 'rh_treenet' in xin_cols and not df_hygro_w.empty and 'rh_treenet' in Xseg_orig.columns:
    ...
    if 'stem' in xin_cols and not df_l2_w.empty and 'stem' in Xseg_orig.columns:
    ...
    if not Xseg_daily.empty:
        for gch in [c for c in GLOBAL_CHANNELS if c in Xseg_daily.columns and c in df_site_w.columns]:
            plot_two_lines(df_site_w['ts_local'], df_site_w[gch].to_numpy(), f'raw site daily {gch}',
                           Xseg_daily.index, Xseg_daily[gch].to_numpy(), 'segment daily mean', gch)

    # Targets
    # local_T: raw 10-min vs target hourly (native)
    if 'local_T' in y_cols and not df_temp_w.empty and 'local_T' in Yseg_orig.columns:
        plot_two_lines(df_temp_w.index, df_temp_w['value'].to_numpy(), 'raw L1 temp (10-min)',
                    Yseg_orig.index, Yseg_orig['local_T'].to_numpy(), 'segment denorm (hourly)', 'local_T')
    # local_RH: raw 10-min vs target hourly (native)
    if 'local_RH' in y_cols and not df_hygro_w.empty and 'local_RH' in Yseg_orig.columns:
        plot_two_lines(df_hygro_w.index, df_hygro_w['value'].to_numpy(), 'raw L1 RH (10-min)',
                    Yseg_orig.index, Yseg_orig['local_RH'].to_numpy(), 'segment denorm (hourly)', 'local_RH')
    # stem: raw LM 10-min vs target hourly (native)
    if 'stem' in y_cols and not df_lm_w.empty and 'stem' in Yseg_orig.columns:
        plot_two_lines(df_lm_w.index, df_lm_w['value'].to_numpy(), 'raw dendrometer LM (10-min)',
                    Yseg_orig.index, Yseg_orig['stem'].to_numpy(), 'segment denorm (hourly)', 'stem_target_LM')

# ---------------- main ----------------
def main():
    p = argparse.ArgumentParser(description='Compare denormalized segment(s) vs raw files on original scale.')
    p.add_argument('--out_root', required=True)
    p.add_argument('--raw_root', required=True, help='Root of server_data directory')
    ...
    p.add_argument('--seg_idx', type=int, default=None, help='Segment index to process (single mode)')
    ...
    _, combo_ids, in_segs, out_segs, seg_ids = load_pickles(args.out_root, args.split)
    ...
    # Preload RAW once per batch using server_data structure
    raw_temp  = read_thermometer_l1(args.raw_root, thermo_id)
    raw_hygro = read_hygrometer_l1(args.raw_root, hygro_id)
    raw_l2    = read_dendrometer_l2(args.raw_root, dendro_id)
    raw_lm    = read_dendrometer_lm(args.raw_root, lm_dendro_id)
    raw_site  = read_site_csv(args.raw_root, args.site_id, args.year)
    ...
    for seg_idx in seg_indices:
        ...
        compare_one_segment(
            year=args.year, site_id=args.site_id, combo_id=args.combo_id, seg_idx=seg_idx,
            ids_row=ids_row, in_min=in_min, in_diff=in_diff, out_min=out_min, out_diff=out_diff,
            ws_utc=ws_utc, we_utc=we_utc,
            Xseg_norm=Xseg_norm, Yseg_norm=Yseg_norm,
            raw_temp=raw_temp, raw_hygro=raw_hygro, raw_l2=raw_l2, raw_lm=raw_lm, raw_site=raw_site,
            fig_out_dir=args.fig_out,
        )

    print(
        f"Wrote comparison figures under: "
        f"{os.path.join(args.fig_out, f'year_{args.year}', f'site_{args.site_id}', f'combo_{args.combo_id}')}\n"
        f"Segments processed: {seg_indices}"
    )
