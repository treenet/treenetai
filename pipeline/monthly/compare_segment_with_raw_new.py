...
#
# INPUT CHANNELS (10): temp_treenet, rh_treenet, stem(L2), tas, tasmax, tasmin, rh, vpd, gh, pr, doy
...
# └─ meteo_data/site_<SITE>.csv                         (daily globals) columns = ['ts','tas','tasmax','tasmin','rh','vpd','gh','pr']
#
# Notes:
# - L1 thermometer/hygrometer and L2 dendrometer files contain only ['ts','value'].
# - LM dendrometer contains ['ts','value','rh','temp'] and must be **resampled to hourly**
#   for comparison with target 'stem'.
# - Daily site CSV is used for globals; we compare daily means of segment vs the CSV.
#
import os
import argparse
import pickle
...
def _read_ftr_columns(raw_path: str, wanted_cols: List[str]) -> pd.DataFrame:
    """Read a feather (.ftr) file, enforce 'ts' presence, and return only the wanted_cols
    (casted to float64) indexed by UTC timestamps.
    """
    ...
    # Validate presence of requested columns
...
# Strict readers per sensor type
...
def read_thermometer_l1(raw_root: str, thermo_id: int) -> pd.DataFrame:
    ...
    df = _read_ftr_columns(path, ['value']).rename(columns={'value': 'value'})
    return df
...
def read_dendrometer_lm(raw_root: str, lm_dendro_id: int) -> pd.DataFrame:
    ...
    # LM contains: ['value','rh','temp'] in addition to 'ts'
    df = _read_ftr_columns(path, ['value','rh','temp'])
    return df
...
def read_site_csv(raw_root: str, site_id: int, year: int) -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    if 'ts' not in df.columns:
        # Allow 'ts_local' fallback if provided
        ts_col = 'ts_local' if 'ts_local' in df.columns else None
    else:
        ts_col = 'ts'
    if ts_col is None:
...
def compare_one_segment(
    ...
    # Resampling for targets (hourly means in local time, back to UTC index)
    ...
    df_temp_hour = to_hourly_mean(df_temp_w[['value']]) if not df_temp_w.empty else df_temp_w
    df_hygro_hour = to_hourly_mean(df_hygro_w[['value']]) if not df_hygro_w.empty else df_hygro_w
    # LM stem is 10-min -> resample to hourly for target comparison
    df_lm_hour = to_hourly_mean(df_lm_w[['value']]) if not df_lm_w.empty else df_lm_w
...
"
                 f"Window: {ws_utc.tz_convert(LOCAL_TZ).strftime('%Y-%m-%d %H:%M')} → {we_utc.tz_convert(LOCAL_TZ).strftime('%Y-%m-%d %H:%M')} (local)")
    ...
    if 'temp_treenet' in xin_cols:
        if not df_temp_w.empty and 'temp_treenet' in Xseg_orig.columns:
    ...
    if 'rh_treenet' in xin_cols:
        if not df_hygro_w.empty and 'rh_treenet' in Xseg_orig.columns:
    ...
    if 'stem' in xin_cols:
        if not df_l2_w.empty and 'stem' in Xseg_orig.columns:
    ...
    if 'local_T' in y_cols:
        if not df_temp_hour.empty and 'local_T' in Yseg_orig.columns:
    ...
    if 'local_RH' in y_cols:
        if not df_hygro_hour.empty and 'local_RH' in Yseg_orig.columns:
    ...
    if 'stem' in y_cols:
        # Use LM hourly mean derived from 10-min LM
        if not df_lm_hour.empty and 'stem' in Yseg_orig.columns:
            plot_two_lines(df_lm_hour.index, df_lm_hour['value'].to_numpy(), 'raw dendrometer LM hourly',
...
def main():
    ...
    p.add_argument('--raw_root', required=True, help='Root of server_data directory (contains subdirs thermometer_l1, hygrometer_l1, dendrometer_l2, dendrometer_lm, meteo_data)')
