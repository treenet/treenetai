#!/usr/bin/env python3
"""
Visualize gap-filling results for test segments with DENORMALIZED values.

For each randomly selected segment, creates a figure with 3 subplots:
1. Temperature (T): raw L1 with gap, model output, ground truth LM - in °C
2. Relative Humidity (RH): raw L1 with gap, model output, ground truth LM - in %
3. Stem Radius: raw L2 with gap, model output, ground truth LM - in μm

Gap regions are shown with shaded background.
All values are denormalized to physical units using segment metadata.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Visualize gap-filling on test segments (denormalized)')
    parser.add_argument(
        '--model-path',
        type=str,
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default='/home/lukovic/data/treenet/gap_filling_visualization_denorm',
...
def denormalize_input(data_normalized, segment_meta, channels=['temp_treenet', 'rh_treenet', 'stem']):
    """
    Denormalize input data using segment metadata.
    
    Args:
        data_normalized: Normalized data (720, 3) or (4320, 3)
        segment_meta: SegmentMetadata object with input_min, input_diff
        channels: List of channel names to denormalize
    
    Returns:
        Denormalized data in original units
    """
    data_denorm = np.zeros_like(data_normalized)
    
    for i, ch in enumerate(channels):
        min_val = segment_meta.input_min[ch]
        diff_val = segment_meta.input_diff[ch]
        data_denorm[:, i] = data_normalized[:, i] * diff_val + min_val
    
    return data_denorm


def denormalize_output(data_normalized, segment_meta, channels=['local_T', 'local_RH', 'stem']):
    """
    Denormalize output data using segment metadata.
    
    Args:
        data_normalized: Normalized data (720, 3)
...
def visualize_sample_denorm(x_original, x_gapped, mask, y_pred, y_truth, 
                            segment_meta, sample_idx, gap_days, output_dir, dpi):
    """
    Visualize a single sample with gap injection and model predictions (DENORMALIZED).
    ...
    x_original_denorm = denormalize_input(x_original_hourly, segment_meta, input_channels)
    x_gapped_denorm = denormalize_input(x_gapped_hourly, segment_meta, input_channels)
    
    # Denormalize output data
    output_channels = ['local_T', 'local_RH', 'stem']
    y_pred_denorm = denormalize_output(y_pred, segment_meta, output_channels)
    y_truth_denorm = denormalize_output(y_truth, segment_meta, output_channels)
    ...
    for ch_idx in range(3):
        ...
        raw_data = x_original_denorm[:, ch_idx]
        gapped_data = x_gapped_denorm[:, ch_idx]
        pred_data = y_pred_denorm[:, ch_idx]
        truth_data = y_truth_denorm[:, ch_idx]
        ...
        # Calculate metrics for gap region (in physical units)
        if np.any(gap_mask):
            ...
            if np.sum(valid) > 10:
                ...
                if ch_idx == 0:  # Temperature
                    metrics_text = f'Gap MAE: {gap_mae:.2f}°C, RMSE: {gap_rmse:.2f}°C, R: {gap_corr:.3f}'
                elif ch_idx == 1:  # RH
                    metrics_text = f'Gap MAE: {gap_mae:.1f}%, RMSE: {gap_rmse:.1f}%, R: {gap_corr:.3f}'
                else:  # Stem
                    metrics_text = f'Gap MAE: {gap_mae:.1f}μm, RMSE: {gap_rmse:.1f}μm, R: {gap_corr:.3f}'
            else:
    ...
    # Overall title
    fig.suptitle(f'Gap-Filling Visualization (Denormalized) - Sample {sample_idx}\n'
    ...
    output_path = output_dir / f'gap_filling_sample{sample_idx}_{gap_days}d_denorm.png'
...
def main():
    ...
    print("="*70)
    print("Gap-Filling Visualization (DENORMALIZED)")
    print("="*70)
    ...
    print(f"Values are now in physical units:")
    print(f"  - Temperature: °C")
    print(f"  - Relative Humidity: %")
    print(f"  - Stem Radius: μm")
